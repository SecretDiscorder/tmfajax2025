...
<head>
    ...
    headerSecond();
    ?>
    <!-- Menggunakan CDN alternatif untuk jquery-textcomplete -->
    ...
    <style>
        /* Tampilan untuk chat bubble seperti WhatsApp */
        .chat_history ul {
            list-style: none;
            padding: 0;
            margin: 0;
...
<body>
            ...
            <nav class="navbar navbar-expand-lg navbar-dark bg-dark text-white">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    ...
                    <span class="btn-group">
                        <a class="btn btn-light my-2 my-sm-0 text-dark"><?php echo $_SESSION['username']; ?></a>
                        <a href="tmf_chat_logout.php" class="btn btn-danger my-2 my-sm-0" type="submit">
    ...
    <div class="container">
        <?php
            // Jika diperlukan, cek status tes untuk membatasi akses fitur chat.
        ...
        <div class="mt-3 mb-3">
            <div id="group_detail">
                ...
                <ul class="list-group">
                    <?php
                        // Menampilkan daftar grup chat berdasarkan session
                        if (is_array($_SESSION['group_id'])) {
                            foreach ($_SESSION['group_id'] as $value) {
                                $query = "SELECT group_name FROM tce_user_groups WHERE group_id = '$value'";
                                $statement = $connect->prepare($query);
                                $statement->execute();
                                $result = $statement->fetchAll();
                                foreach ($result as $row) {
                                    echo '<li class="list-group-item" name="group_chat_' . $value . '" id="group_chat_' . $value . '">
                                            <div class="d-flex">
                                                <span style="color:#aaa"><i class="fas fa-users fa-2x"></i></span>&nbsp;' . $row['group_name'] . '
                                            </div>
                                          </li>';
                                }
                            }
                        } else {
                            echo '<li class="list-group-item" name="group_chat_' . $_SESSION['group_id'] . '" id="group_chat_' . $_SESSION['group_id'] . '">
                                    <span style="color:#aaa"><i class="fas fa-users fa-lg"></i></span>&nbsp;' . $_SESSION['group_id'] . '
                                  </li>';
                        }
                    ?>
                </ul>
    ...
    <div id="group_chat_dialog" class="this_chat_dialog">
        <div id="group_chat_history" style="height:400px; border:1px solid #ccc; overflow-y: scroll; margin-bottom:24px; padding:16px;">
            <!-- Riwayat chat grup akan ditampilkan di sini -->
        </div>
            ...
            if (is_array($_SESSION['group_id'])) {
                foreach ($_SESSION['group_id'] as $value) {
                    echo "$('#group_chat_$value').click(function(){
                            $('input#group_id').val($value);
                            $('#group_chat_dialog').dialog('open');
                            $('#is_active_group_chat_window').val('yes');
                            fetch_group_chat_history($value);
                          });";
                }
            } else {
                echo "$('#group_chat_" . $_SESSION['group_id'] . "').click(function(){
                        $('#group_chat_dialog').dialog('open');
                        $('#is_active_group_chat_window').val('yes');
                        fetch_group_chat_history(" . $_SESSION['group_id'] . ");
                      });";
            }
        ?>
