 ...
 */
define('K_DATABASE_NAME', 'bimapust_perisai');

/**
 * database user name
 */
define('K_DATABASE_USER_NAME', 'bimapust_perisai');

/**
 * database user password
 */
define('K_DATABASE_USER_PASSWORD', 'perisaismanca');
...
$connect = new PDO(
    "mysql:host=" . K_DATABASE_HOST . ";dbname=" . K_DATABASE_NAME . ";charset=utf8mb4", 
    K_DATABASE_USER_NAME, 
    K_DATABASE_USER_PASSWORD, 
    $options
);
try {
    $query = "SELECT * FROM " . K_TABLE_USERS;
    $statement = $connect->prepare($query);
    $statement->execute();
    $results = $statement->fetchAll();
} catch (PDOException $e) {
    echo "Error: " . $e->getMessage();
    // Atau simpan error tersebut ke file log:
