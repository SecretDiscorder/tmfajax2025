...
function F_printTestResultStat($data, $nextorderdir, $order_field, $filter, $pubmode = false, $stats = 1)
{
	...
	$ret = '';
    //$ret = '<div id="table_field_select" style="text-align:right;width:180px;border:1px solid #bbb;padding:5px;border-radius:7px">Waktu Mulai : <input id="waktu_mulai_input" type="checkbox" checked="checked" /><br/>Durasi : <input id="durasi_input" type="checkbox" checked="checked" /><br/>Sisa durasi : <input id="sisa_durasi_input" type="checkbox" checked="checked" /></div><div id="printMe" href="#" style="display:inline; padding-right:3px"><input type="button" id="unlock" value="Export Excel"></input></div><div id="exportPDF" href="#" style="display:inline; padding-right:17px"><input type="button" id="extendtime" value="Print / Export PDF"></input></div><div id="tabel_hasil_test"><html><body><p id="nm_ujian"></p><p id="kelas"></p><br/>';
//    $ret = '<div id="table_field_select" style="text-align:right;width:180px;border:1px solid #bbb;padding:5px;border-radius:7px">Waktu Mulai : <input id="waktu_mulai_input" type="checkbox" checked="checked" /><br/>Durasi : <input id="durasi_input" type="checkbox" checked="checked" /><br/>Sisa durasi : <input id="sisa_durasi_input" type="checkbox" checked="checked" /></div><div id="printMe" href="#" style="display:inline; padding-right:3px">
//<a id="export_excel" download="" href="">Export Excel</a>
//</div><div id="exportPDF" href="#" style="display:inline; padding-right:17px"><input type="button" id="extendtime" value="Print / Export PDF"></input></div><div id="tabel_hasil_test"><html><body><p id="nm_ujian"></p><p id="kelas"></p><br/>';

	//$ret = '<p id="kelas"></p>'.K_NEWLINE;
    $ret .= '<table id="userselect" class="userselect" style="border-spacing: 0; border-collapse: collapse;">'.K_NEWLINE;
    $ret .= '<tr><thead>'.K_NEWLINE;
    $ret .= '<th id="checkbox">&nbsp;</th>'.K_NEWLINE;
    $ret .= '<th width="25px">NO</th>'.K_NEWLINE;
...
//    $ret .= '<th width="70px" id="time_begin" title="'.$l['h_time_begin'].'">'.strtoupper($l['w_time_begin']).'</th>'.K_NEWLINE;
//    $ret .= '<th id="test_time" title="'.$l['h_test_time'].'">'.strtoupper($l['w_time']).'</th>'.K_NEWLINE;
    // $ret .= F_select_table_header_element('kelas', $nextorderdir, 'kelas', 'KELAS', $order_field, $filter);
    $ret .= F_select_table_header_element('testuser_test_id', $nextorderdir, $l['h_test'], strtoupper($l['w_test']), $order_field, $filter);
    if (!$pubmode) {
        $ret .= F_select_table_header_element('user_name', $nextorderdir, $l['h_login_name'], strtoupper('username'), $order_field, $filter);
        $ret .= F_select_table_header_element('user_firstname', $nextorderdir, $l['h_firstname'], strtoupper($l['w_firstname']), $order_field, $filter);
    }
    $ret .= F_select_table_header_element('total_score', $nextorderdir, $l['h_score_total'], strtoupper('Nilai'), $order_field, $filter);
    $ret .= F_select_table_header_element('total_score', $nextorderdir, $l['h_score_total'], strtoupper('Test User ID'), $order_field, $filter);
	//$ret .= F_select_table_header_element('total_score', $nextorderdir, $l['h_score_total'], $l['w_score'], $order_field, $filter);
    if ($stats > 0) {
        $ret .= '<th title="'.$l['h_answers_right'].'">'.$l['w_answers_right'].'</th>'.K_NEWLINE;
        $ret .= '<th title="'.$l['h_answers_wrong'].'">'.$l['w_answers_wrong'].'</th>'.K_NEWLINE;
        $ret .= '<th title="'.$l['h_questions_unanswered'].'">'.str_replace(' ','<br/>',$l['w_questions_unanswered']).'</th>'.K_NEWLINE;
        $ret .= '<th title="'.$l['h_questions_undisplayed'].'">'.str_replace(' ','<br/>',$l['w_questions_undisplayed']).'</th>'.K_NEWLINE;
        //$ret .= '<th title="'.$l['h_questions_unrated'].'">'.$l['w_questions_unrated'].'</th>'.K_NEWLINE;
    }
    //maman $ret .= '<th title="'.$l['w_status'].' ('.$l['w_time'].' ['.$l['w_minutes'].'])">'.$l['w_status'].'<br/>('.$l['w_time'].' ['.$l['w_minutes'].'])</th>'.K_NEWLINE;
//    $ret .= '<th id="sisa_durasi" title="'.$l['w_status'].' ('.$l['w_time'].' ['.$l['w_minutes'].'])">'.strtoupper($l['w_status'].'<span style="display:block"> ( Sisa Durasi )</span></th>').K_NEWLINE;
    //maman
    //$ret .= '<th title="'.$l['h_testcomment'].'">'.$l['w_comment'].'</th>'.K_NEWLINE;
    //$ret .= '<th></th>'.K_NEWLINE;
    $ret .= '</tr></thead><tbody>'.K_NEWLINE;
	//echo print_r($data['testuser']);
    foreach ($data['testuser'] as $tu) {
        ...
        if (!$pubmode or F_getBoolean($tu['test']['test_report_to_users'])) {
            $ret .= '<td style="text-align:'.$tdalign.'"><a href="tmf_show_offline_sheet.php?testuser_id='.$tu['id'].'&amp;test_id='.$tu['test']['test_id'].'&amp;user_id='.$tu['user_id'].'" title="'.$l['h_view_details'].'">'.$tu['num'].'</a></td>'.K_NEWLINE;
        } else {
            $ret .= '<td>'.$tu['num'].'</td>'.K_NEWLINE;
        }
        //$ret .= '<td id="time_begin" style="text-align:left; width:70px">'.str_replace(' ','&nbsp;<span style="color:blue;display:block">',$tu['testuser_creation_time']).'</span></td>'.K_NEWLINE;
        //$ret .= '<td style="text-align:center;">'.$tu['testuser_end_time'].'</td>'.K_NEWLINE;
        //$ret .= '<td id="test_time" style="text-align:left">'.$tu['time_diff'].'</td>'.K_NEWLINE;
        $passmsg = '';
        if ($tu['passmsg'] === true) {
            $passmsg = ' title="'.$l['w_passed'].'" style="background-color:#dff0d8;border-left:1px solid #aaa"';
        } elseif ($tu['passmsg'] === false) {
            $passmsg = ' title="'.$l['w_not_passed'].'" style="background-color:#f2dede;border-left:1px solid #aaa"';
        }
        if ($pubmode) {
            $ret .= '<td style="text-align:'.$tdalign.';">'.$tu['test']['test_name'].'</td>'.K_NEWLINE;
        } else {
		//kelas
$sqlr = 'SELECT kelas FROM tce_users WHERE user_id='.$tu['user_id'];
if($rr = F_db_query($sqlr, $db)){
	while ($mr = F_db_fetch_array($rr)){
	    $ret .= '<td id="kelas" style="text-align:'.$tdalign.';">'.$mr['kelas'].'</td>'.K_NEWLINE;
	}
}

            $ret .= '<td id="nm_ujian" style="text-align:'.$tdalign.';"><a href="tce_edit_test.php?test_id='.$tu['test']['test_id'].'">'.$tu['test']['test_name'].'</a></td>'.K_NEWLINE;
            //maman $ret .= '<td style="text-align:'.$tdalign.';"><a href="tce_edit_user.php?user_id='.$tu['user_id'].'">'.$tu['user_name'].'</a></td>'.K_NEWLINE;
            //$ret .= '<td style="text-align:'.$tdalign.';">&nbsp;'.$tu['user_lastname'].'</td>'.K_NEWLINE;
            $ret .= '<td style="text-align:'.$tdalign.';">&nbsp;'.$tu['user_name'].'</td>'.K_NEWLINE;
			$ret .= '<td style="text-align:'.$tdalign.';">&nbsp;'.$tu['user_firstname'].'</td>'.K_NEWLINE;
        }
		//maman ret
        $ret .= '<td'.$passmsg.' class="numeric">'.F_formatPercentageAlt($tu['total_score_perc'], false).'</td>'.K_NEWLINE;

//	$testhash = base64_encode('L J K '.$tu['test']['test_id'].' User '.$tu['user_id']);
	$testhash = $tu['test']['test_id'].'-'.$tu['user_id'];

		$ret .= '<td class="numeric">'.$testhash.'</td>'.K_NEWLINE;
        if ($stats > 0) {
	/*
...
/**
        if ($tu['locked']) {
            $ret .= '<td id="sisa_durasi" style="background-color:#f2dede;border-left:1px solid #aaa;text-align:left">'.$l['w_locked'];
        } else {
            $ret .= '<td id="sisa_durasi" style="background-color:#dff0d8;border-left:1px solid #aaa;text-align:left">'.$l['w_unlocked'];
        }
        if ($tu['remaining_time'] < 0) {
            $ret .= '<br/>( '.substr_replace($tu['remaining_time'],'',0,1).' menit )';
        }
**/
        $ret .= '</td>'.K_NEWLINE;
    ...
    $ret .= '<td id="berhasil" colspan="'.$colspan.'" style="text-align:'.$tdalign.';font-weight:bold;padding-right:10px;padding-left:10px;';
    if ($data['passed_perc'] > 50) {
        $ret .= ' background-color:#dff0d8;"';
    } else {
        $ret .= ' background-color:#f2dede;"';
    }
    ...
    foreach ($data['statistics'] as $row => $col) {
        if (in_array($row, $printstat)) {
            ...
            $ret .= '<th id="statistik" colspan="'.$scolspan.'" style="text-align:'.$tdalignr.';border-bottom:1px solid #aaa;padding-right:17px;background:#d9edf7">'.$l['w_'.$row].'</th>'.K_NEWLINE;
            //$ret .= '<th id="statistik" colspan="'.$scolspan.'" style="text-align:'.$tdalignr.';border-bottom:1px solid #bbb;border-right:1px solid #bbb;padding-right:17px;background:#d9edf7">'.$l['w_'.$row].'</th>'.K_NEWLINE;
            ...
            $ret .= '<td class="sisa_durasi_bawah" id="sisa_durasi" colspan="2">&nbsp;</td>'.K_NEWLINE;
            $ret .= '</tr>';
        }
    }
    $ret .= '</tbody></table></body></html></div>'.K_NEWLINE;
    return $ret;
}
...
function F_printUserTestStat($testuser_id)
{
	...
	$jum=0;
		...
		if($m['testlog_score']>0){
			$jml_text_benar++;
			$jml_text_nilai += $m['testlog_score'];
		}
	    }
		...
		if($m['testlog_score']>0){
			$jml_mcma_benar++;
			$jml_mcma_nilai += $m['testlog_score'];
		}
	    }
		...
		if($m['testlog_score']>0){
			$jml_order_benar++;
			$jml_order_nilai += $m['testlog_score'];
		}
	    }
    ...
    $pros=$jml_mcsa_benar/$jml_mcsa*100;
    echo '<div class="row" style="border:none"><span class="label"><b>Pilihan Ganda</b></span></div><div class="row" style="border:none"><span class="label">- Benar:</span><span class="formw">'.$jml_mcsa_benar.' / '.$jml_mcsa.' ('.$pros.'%)</span></div>';

	if(isset($_REQUEST['test_id']) and ($_REQUEST['test_id'] > 0)){
		$test_id = intval($_REQUEST['test_id']);
	}else{
		$sql2 = 'SELECT * FROM tce_tests ORDER BY test_begin_time DESC LIMIT 1';
		    if ($r2 = F_db_query($sql2, $db)) {
		        while ($m2 = F_db_fetch_array($r2)) {
				$test_id=$m2['test_id'];
			}
		   }
	}

	$sql3 = 'SELECT * FROM tce_tests WHERE test_id='.$test_id;
	    if ($r3 = F_db_query($sql3, $db)) {
	        while ($m3 = F_db_fetch_array($r3)) {
			$test_score_right=$m3['test_score_right'];
			$kkm=$m3['test_score_threshold'];
		}
	   }

	$poin=$jml_mcsa_benar*$test_score_right;

	echo '<div class="row" style="border:none"><span class="label">- Nilai:</span><span class="formw">'.$poin.'</span></div>';
	//if($jml_text>0){
	        echo '<div class="row" style="border:none"><span class="label"><b>Jawaban Ganda</b></span></div><div class="row" style="border:none"><span class="label">- Nilai:</span><span class="formw">'.$jml_mcma_nilai.'</span></div>';
	        echo '<div class="row" style="border:none"><span class="label"><b>Ordering</b></span></div><div class="row" style="border:none"><span class="label">- Nilai:</span><span class="formw">'.$jml_order_nilai.'</span></div>';
	        echo '<div class="row" style="border:none"><span class="label"><b>Essay</b></span></div><div class="row"><span class="label">- Nilai:</span><span class="formw">'.$jml_text_nilai.'</span></div>';

		$jml_all_nilai=$poin+$jml_text_nilai+$jml_mcma_nilai+$jml_order_nilai;

		if($jml_all_nilai>$kkm){
			$ket="TUNTAS";
		}else{
			$ket="REMIDI";
		}

	        echo '<div class="row"><span class="label"><b>Jumlah Nilai</b></span><span class="formw">'.$jml_all_nilai.'</span></div>';
	//}
	echo '<div class="row"><span class="label">Keterangan:</span><span class="formw">'.$ket.'</span></div>';
	//echo $jml_text;
	//echo $jml_text_benar;
	//echo $jml_text_nilai;
	//echo $jml_mcsa;
	...
	//echo $jml_mcma;
        //echo $jml_mcma_nilai;
	//echo $jml_order.'_';
	//echo $jml_order_benar.'_';
        //echo $jml_order_nilai.'_';
