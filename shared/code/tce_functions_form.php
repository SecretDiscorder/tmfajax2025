...
function F_submit_button_confirm($name, $value, $confirm = "", $title = "")
{
    // Ensure that $confirm is well-formed before embedding it into HTML.
    $confirm = htmlspecialchars($confirm, ENT_QUOTES, 'UTF-8');
    
    // Echo the input field with the correct attributes.
    echo '<input type="submit" name="' . htmlspecialchars($name, ENT_QUOTES, 'UTF-8') . '" 
...
$str .= sprintf(
    ' name="%s" id="%s" value="%s" size="20" maxlength="%d" title="%s" />',
    htmlspecialchars($field_name ?? '', ENT_QUOTES, 'UTF-8'),  // Use null coalescing to default to an empty string if null
    htmlspecialchars($field_name ?? '', ENT_QUOTES, 'UTF-8'),  // Same for other fields
    htmlspecialchars($value ?? '', ENT_COMPAT, $l['a_meta_charset']),
    (int) $maxlen,  // Ensure $maxlen is cast to an integer
    htmlspecialchars($description ?? '', ENT_QUOTES, 'UTF-8')  // Same for description
);
