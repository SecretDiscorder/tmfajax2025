...
$thispage_title_icon = '<i class="fas fa-user-cog"></i> ';
...
// check password
            if (!empty($newpassword) or !empty($newpassword_repeat)) { // update password
                if ($newpassword == $newpassword_repeat) {
                    // Assign the password directly (no encoding)
                    $user_password = $newpassword;
                    
                    // update OTP key (this can still be hashed or randomized as per original logic)
                    $user_otpkey = F_getRandomOTPkey();
                } else { // print message and exit
                    F_print_error('WARNING', $l['m_different_passwords']);
                    $formstatus = false;
