...
$thispage_title_icon = '<i class="fas fa-mail-bulk"></i> ';
...
$baseDir = K_PATH_CACHE; // You can change this to any base directory
$dirToCheck = 'OMR'; // You can change this to check other subdirectories dynamically
    ...
    if ($dirhdl !== false) {
        while ($file = readdir($dirhdl)) {
            if (($file != '.') && ($file != '..')) {
                ...
                // Only check if it's a directory
                if (is_dir($filename)) {
                    $dirs[$filename] = 'OMR/'.$file.'/';
                }
            }
        }
        closedir($dirhdl); // Close the directory handle
